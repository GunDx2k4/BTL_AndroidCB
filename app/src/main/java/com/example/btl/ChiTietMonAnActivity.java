...
import com.example.btl.BookmarkDAO;
...
public class ChiTietMonAnActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        monAnId = intentct.getIntExtra("id",-1);

        if (monAnId == -1) {
            Toast.makeText(this, "Lỗi không tìm thấy món ăn " + monAnId, Toast.LENGTH_SHORT).show();
            return;
        }

        var db = new MonAnDAO(this);
        var monAn = db.getMonAnById(monAnId);
